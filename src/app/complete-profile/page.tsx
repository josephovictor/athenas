...
export default function CompleteProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update the JWT so middleware gate is lifted
      await update({ profileCompleted: true, displayName: form.name });
      ...
      const home = roleHome[(session?.user as any)?.role] ?? "/login";
      router.push(home);
    } catch {
  ...
  return (
    <div className="flex h-screen overflow-hidden items-center justify-center bg-gray-50">
      <div className="w-full max-w-[440px] bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col">
