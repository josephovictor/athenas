...
export default function DashboardLayout({
  ...
  // Define the navigation items in a clean array
  const navItems = [
      ...
      <div className="h-screen overflow-hidden bg-white flex text-[var(--color-brand-dark)]">
        ...
        <aside className="w-64 bg-[var(--color-brand-dark)] text-white flex flex-col z-20 fixed h-screen">
          ...
          <nav className="px-4 pb-6 space-y-2 text-sm font-medium border-b border-white/10">
            {navItems.map((item) => {
              // Check if active (exact match for Home, or startsWith for subpages like /dashboard/seminar/...)
              ...
              return (
                <Link 
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${
                    isActive 
                      ? "bg-[var(--color-brand-teal)] text-white shadow-sm" // The green active state
                      : "text-[var(--color-brand-light)] hover:bg-white/10 hover:text-white" // The grey inactive/hover state
                  }`}
                >
